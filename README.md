...
#### BOS/EOS が bigram に含まれていない

現在、BOS（文頭）と EOS（文末）は bigram モデルに含まれていません。BOS/EOS ノードは `word_id` を持たず、これらに接続するエッジはすべてデフォルトコストが使用されます。そのため、「文頭に来やすい単語」「文末に来やすい単語」の情報が活用されていません。

#### 単一最良パスのみ（k-best 非対応）

ビタビアルゴリズムで最もコストの低い 1 本のパスだけを返しています。各ノードに対して最良の前ノードを 1 つだけ記録する `prevmap: HashMap<&WordNode, &WordNode>` を使用しており、分節の区切り方が異なる候補パスを列挙する仕組みがありません。

ユーザーに提示される `Vec<Vec<Candidate>>` の内側の候補は、最良パスで決まった分節位置における「同じ読み・同じ位置の別漢字候補（辞書引きの結果）」であり、**分節パターンが異なる別の変換候補ではありません**。
...
#### BOS/EOS の bigram への導入

BOS/EOS に word_id を振り、bigram の訓練データに含めることで、文頭・文末の文脈情報を活用できるようにする。

- **期待される効果**: 文末の「しよう」→「使用」の誤変換を、EOS との bigram で「しよう」のスコアを上げることで抑制できる可能性がある。文頭に来やすい「私は」「今日は」なども精度向上が見込める。
- **実装コスト**: 低い。BOS/EOS に word_id を付与し、コーパスのパース時に除去しないようにするだけ。

#### k-best 解の導出

ビタビアルゴリズムを拡張し、上位 k 個の最良パスを列挙できるようにする。

- **期待される効果**: 分節の区切り方が異なる候補を提示できる（例: 「きょうは」→「今日/は」vs「今日は」一語）。現状は最良パスの分節区切りが固定されており、その中の漢字候補しか選べない。
- **実装方法**: k-best Viterbi または A* アルゴリズム。各ノードに対して上位 k 個の前ノードを記録する必要がある。
